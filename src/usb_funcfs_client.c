...
#define STR_INTERFACE_ "SDB Interface"
...
static int read_control(struct usb_handle *usb)
{
    ...
    const struct usb_functionfs_event read_events[5];
    int i, read_events_cnt, ret;

    /* read events from control */
    ret = sdb_read(usb->control, &read_events, sizeof(read_events));
    if (ret < 0) {
        if (errno == EAGAIN) {
            sleep(1);
            return ret;
    ...
    read_events_cnt = ret / sizeof read_events[0];
    for (i = 0; i < read_events_cnt; i++) {
        switch (read_events[i].type) {
            case FUNCTIONFS_RESUME:
            case FUNCTIONFS_ENABLE:
            	D("FFSEvent %s\n", names[read_events[i].type]);
                sdb_mutex_lock(&usb->lock);
                usb->ffs_enabled = 1;
 ...
 *
 * @returns amount of bytes written or error code (negative)
 */
 ...
 *
 * @returns amount of bytes read or error code (negative)
 */
