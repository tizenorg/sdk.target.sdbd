...
enum tzplatform_get_env_error_status {
    ERROR_TZPLATFORM_GENERAL = 1,
    ERROR_TZPLATFORM_INVALID_VARIABLES = 2,
};
...
void get_tzplatform_env(int fd, void *cookie) {
    ...
    if (env_id != _TZPLATFORM_VARIABLES_INVALID_) {
        ...
        if (env_value) {
            ...
            snprintf(buf, sizeof(buf), "%s", ERROR_TZPLATFORM_GENERAL);
        }
    } else {
        D("environment name (%s) is invalid\n", env_name);
        snprintf(buf, sizeof(buf), "%s", ERROR_TZPLATFORM_INVALID_VARIABLES);
    }
    writex(fd, buf, strlen(buf));
    free(env_name);
    sdb_close(fd);
